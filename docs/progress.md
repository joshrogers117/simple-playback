...
- **Active phase**: A and D complete; B mostly done (B1–B5, B12 shipped; B6 mostly done; B8 pure-logic recommendation + inspector hint shipped; B14 partial); Phase C substantially done (C1–C6 + C-banner all in; **C7 core landed in session 16**: AssetFingerprinter + MediaReference kind/fingerprint fields + importer wiring + MediaResolver waterfall pure-logic; cancelled transcode/encode partial files now self-clean); **C9 first slice in session 16**: relink-folder Fix handler on Pre-Show media.files row; **Phase E**: E1 done + E1+ energy adapter (session 16 added the asset-library `media.files` row + stale-fingerprint detection); E2 done; E3 done; E3+ done (DroppedFrameCounter pure-logic + PlaybackController detection + status-bar chip + ShowController log debounce — late-take detection still deferred); E4 done; E5 done (TakeHistory in-memory v1 + viewer sheet — replay scrub deferred); E6 done; E7 done (crash recovery — Restore/Discard banner on autosave newer than Show.json); E8 done. Remaining E items: more E1+ macOS-condition adapters (Spotlight / DND / Time Machine / screensaver — fragile / privacy-blocked); late-take detection (E3+ tail), E8 read-only-mode banner option, E9 (Director View), E10 (Workspaces), E11 (brightness adapt key). Remaining Phase C items: C7d Bundle for Travel; C8 folder bookmarks; C9 banner/UX polish; C10/C11 thumbnails; C12-C15 audio.
- **Last commit**: asset-library: relink-folder Fix handler on Pre-Show media.files row (C9 first slice)
- **Branch**: `development`
...
- [~] C7: Asset library — linked vs managed media; security-scoped bookmark + path + content hash + size + mtime
  ...
  - [ ] C7d: Bundle for Travel — copy linked media into `<bundle>/Media/` and rewrite `MediaReference.kind` to `.managed`. UI surface deferred.
...
- [~] C9: Missing-media UX — non-modal banner, per-clip Locate, project-level Relink folder, hash-based auto-relink
  ...
  - [ ] Per-slide Locate context menu for one-shot file picks.
