...
- **Last commit**: postshow: add CSV exporter for PostShowSummary (post-show v2 third slice)
- **Branch**: `development`
...
- [~] G-postShow: **Post-Show Summary** (`docs/v2/post_show_summary.md`). First slice shipped session 29 (5 commits, 749 → 807 tests, +58). Sub-tasks 1–5 done (`.cueEnded` log delta + emit, `.takeLatency` log delta + emit-on-every-verdict, `Services/PostShowSummary.swift` reducer, `Services/PostShowMarkdownExporter.swift`, `Services/PostShowCSVExporter.swift`). **Sub-task 6** (sheet UI + toolbar wiring) deferred — needs operator UX input on Q1 (date-picker scope: since-launch vs today's-log vs operator-picked range), Q5 (format default — Markdown / CSV / both), Q6 (sensitive-data redaction — default posture + toggle visibility). **Sub-task 7** (REF / audio-device / lock-file system-event log integration — Q4-B widening) deferred — touches B6b + E1 + E8 subsystems; each push-to-show-log delta needs operator review of which events warrant a row. The pure-logic core is invariant under these UX choices; the future-session pickup is "open the v2 doc, get answers, build the sheet, wire the system-event push." Decisions made this session captured in `docs/decision_log.md` 2026-05-08 session-29 entry.
