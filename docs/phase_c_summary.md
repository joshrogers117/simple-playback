...
**Still deferred for v1.1.x** — the Option-J `AudioPump` `tracks(withMediaType:)` site; track-property-level async bridges; cross-host folder-bookmark scenario rehearsal (sandbox-vs-NAS, security-scoped folder access on a moved bundle).
