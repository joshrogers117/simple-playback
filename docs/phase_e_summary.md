...
**Status (session 25 — 2026-05-08) — ShowLog persistence-suspended banner shipped (E3 hardening from F1 sweep)**. The on-disk show-log writer used to silently drop persistence on the first failed write (read-only volume, NAS timeout, full disk, permission flip mid-show) — `fileURL` was cleared, in-memory append kept working, and the operator had no signal until post-show. Spec §3.16 lists the on-disk log as a v1 reliability artifact, so silent drop is not acceptable. Added `@Published private(set) var persistenceState: .healthy | .suspended(reason: String)`. Failures flip the state to `.suspended(error.localizedDescription)`; subsequent appends short-circuit (fileURL is cleared so we don't hammer the failing volume); in-memory `events` accumulation continues so Export CSV still works; operator re-arms by calling `setFileURL` again, which resets state to `.healthy`. `ShowLogView` renders an orange "Disk log paused — events still captured in memory" banner with the reason text between the filter toolbar and the row list. 4 new pin tests in `ShowLogTests` (initial healthy, seed-fail transition, append-fail transition + counter still moves, re-arm reset). 741 → 745 tests.
